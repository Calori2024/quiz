...
$(document).ready(function () {
  ...
  $("._1week").text(`${safeWeightLossRate} `);
  $("._2weeks").text(`${safeWeightLossRate * 2} `);
    ...
    $("#timeResult").text(
      `(${
        formattedTimeResult.months === 0
          ? formattedTimeResult.weeks !== 0
            ? `${formattedTimeResult.weeks}vko`
            : ""
          : formattedTimeResult.weeks === 0
          ? `${formattedTimeResult.months}kk`
          : `${formattedTimeResult.months}kk ${formattedTimeResult.weeks}vko`
      })`
    );
    $("#dateResult").text(
      `${calculateFutureDate(
        formattedTimeResult.months,
        formattedTimeResult.weeks
      )}`
    );
    $("#timeAndDate").val(
      `${calculateFutureDate(
...
//Result quiz tab switcher start
$("#oneTimePayment").on("click", function () {
  $(".select-plan_tarifs-wrap.is-subscription").removeClass("is-active");
  $(".select-plan_tarifs-wrap.is-one-time").addClass("is-active");
});

$("#subscription").on("click", function () {
  $(".select-plan_tarifs-wrap.is-one-time").removeClass("is-active");
  $(".select-plan_tarifs-wrap.is-subscription").addClass("is-active");
});
//Result quiz tab switcher end
