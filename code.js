...
$(document).ready(function () {
  ...
  const $timeResultText = $("#timeResultText");
  ...
  function formatMonths(weeks) {
    ...
    let result = `${months} kk.`;
    if (left_weeks > 0) {
      result += ` ${left_weeks} vko.`;
    }
    return result;
  }
  ...
  function changeValues() {
    ...
    $recommendedWeight.val(`${lowerWeight} - ${upperWeight} kg`);
    ...
    $timeResult.val(`${formattedTimeResult}`);
    $timeResultText.text(`${formattedTimeResult}`);
  ...
  function checkInput(input) {
    ...
    switch (name) {
      ...
      case "allergy":
        ...
        console.log(`${hasBadAllergy} or ${hasOther}`);
